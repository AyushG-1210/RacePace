...
def identify_stints(df):
    ...
    Logic: New Stint = Change in Compound OR Reset in TireLife.
    """
    print("--> [PACKAGE] Identifying Stints...")
    
    # 1. Did the tire age reset? (e.g., 20 -> 1)
    # We use .shift(1) to compare current row with previous row
    df['Tire_Reset'] = df['TireLife'] < df['TireLife'].shift(1)
    ...
    df['Stint_ID'] = (df['Tire_Reset'] | df['Compound_Change']).cumsum().fillna(0).astype(int)
...
def add_fuel_correction(df, initial_fuel_kg=110, time_loss_per_kg=0.035):
    """
    Calculates the 'True' Tire Pace by removing the fuel weight advantage.
    """
...
def filter_warmup_laps(df):
    ...
    # Group by Stint and remove the entry with the lowest TireLife (the start)
    # Alternatively, just drop the first row of the group.
    
    # Logic: Keep rows where TireLife is NOT the minimum of that stint
    # (This handles cases where you might enter track on old tires)
...
def process_packaging(clean_laps, weather_data):
    ...
    cols = ['Stint_ID', 'LapNumber', 'TireLife', 'Compound', 
